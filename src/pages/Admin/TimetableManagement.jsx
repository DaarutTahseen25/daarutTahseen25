...
import { Pencil, Trash2, Plus, User, Wifi, Clock3 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 sm:mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-medium text-gray-900 mb-1 sm:mb-2">
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-3 bg-teal-500 text-white font-medium rounded-lg hover:bg-teal-600 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base whitespace-nowrap"
          >
            <Plus size={18} className="sm:w-5 sm:h-5" />
            <span className="hidden xs:inline">Add Timetable Entry</span>
            <span className="xs:hidden">Add Entry</span>
          </button>
        </div>
        ...
        <div className="space-y-4 sm:space-y-6 md:space-y-8">
          {Object.entries(groupedEntries).map(([date, dateEntries]) => (
            ...
            >
              {/* Date Header */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-gray-200">
                <div>
                  <h2 className="text-lg sm:text-xl font-semibold text-gray-900">
                    {date}
                  </h2>
                  <p className="text-xs sm:text-sm text-gray-500 mt-1">
                    {getEntriesByDate(date)} entries scheduled •{" "}
                    {getTotalStudents(date)} total students
                  </p>
                </div>
                <div className="mt-2 sm:mt-0 flex items-center gap-2 text-xs sm:text-sm">
                      ...
                      <div className="flex flex-col items-start gap-3 sm:gap-4 flex-1 min-w-0">
                        <div className="flex gap-2 sm:gap-3 w-full">
                          ...
                          <div className="flex-shrink-0">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#2462FF33] rounded-lg flex items-center justify-center">
                              <Clock3
                                size={16}
                                className="sm:w-[18px] sm:h-[18px] text-[#2462FF]"
                              />
                            </div>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                            ...
                            <span
                              className={`px-2 py-0.5 sm:py-1 text-xs font-medium rounded ${entry.tagColor}`}
                            >
                              {entry.tag}
                            </span>
                          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className="flex py-2 sm:py-3 items-center gap-1 text-xs sm:text-sm text-gray-700">
                            <User
                              size={14}
      ...
      {/* Dialog using shared Dialog component */}
          ...
          >
            ...
            <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 pt-3 sm:pt-4">
              <button
                ...
                className="flex-1 px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors text-sm sm:text-base"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 px-4 sm:px-6 py-2 sm:py-3 bg-teal-500 text-white font-medium rounded-lg hover:bg-teal-600 transition-colors text-sm sm:text-base"
              >
